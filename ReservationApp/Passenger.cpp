	...
	if (!flist.empty())
	{
		...
		if (choice == "y" || choice == "Y")
		{
			...
			do
			{
				...
				if ((this->name.length() <= 10) && (Helpers::checkString(this->name)))
				{
					flag = true;
				}
				else
			...
			do
			{
				...
				if ((this->surname.length() <= 10) && (Helpers::checkString(this->surname)))
				{
					flag = true;
				}
				else
			...
			do
			{
				...
				else if (!Passenger::uniquePass(atoi(temp.c_str())))
				{
			...
			do
			{
				...
				if ((this->nationallity.length() <= 10) && (Helpers::checkString(this->nationallity)))
				{
					flag = true;
				}
				else
		...
		{ // existing customer
			...
			if (!(Passenger::uniquePass(atoi(temp.c_str()))))
			{
				for (std::list<Passenger>::iterator i = passengerList.begin(); i != passengerList.end(); ++i)
				{
		...
		// check input
		while (!Helpers::checkNumber(temp) && !Flight::flightExists(atoi(temp.c_str())))
		{
		...
		*/
		...
		passengerList.push_back(*this); // add object to passengerList
	}
	else
	{
...
void Passenger::bookFromQueue(int num)
{ //is called only from cancel()

	bool flag = true;
	Passenger queueFront; // object of the customer added last
...
void Passenger::cancel()
{
	...
	if (!passengerList.empty())
	{
		...
		// check input
		while (!Helpers::checkNumber(passport))
		{
			cout << "Please insert a valid passport number!" << endl;
			getline(cin, passport);
			cout << endl;
		...
		while (!Passenger::displayPersonInfo(atoi(passport.c_str())))
		{
...
bool Passenger::displayPersonInfo(int passport)
{
...
bool Passenger::uniquePass(int passport)
{
	for (std::list<Passenger>::iterator i = passengerList.begin(); i != passengerList.end(); ++i)
	{
...
bool Passenger::checkTime2(Time tLeaving, Time tArriving)
